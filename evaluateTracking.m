...
%     stI.Xi(find(stI.Xi(:)))=-1;
    ...
    % err = {};
    % err{1} = seqCnt; err{2} = gtInfoSingle; err{3} = stI;
    a = struct2mat(gtInfoSingle(seqCnt).gtInfo,0); b = struct2mat(stI,1);
    [mets, mInf]=CLEAR_MOT_HUN(a,b,0.5,0);
...
if eval2D
    ...
    a = struct2mat(gtInfo,0); b = struct2mat(stInfo,1);
    [m2d, mInf]=CLEAR_MOT_HUN(a,b,0.5,0);
    allMets.bmark2d=m2d;
