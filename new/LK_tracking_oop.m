...
% modified by Yu Xiang modified by Quoc L.

function tracker = LK_tracking_oop(frame_id, dres_image, dres_det, tracker)

% current frame + motion
% J = dres_image.Igray{frame_id};
J = dres_image.getFrameGray(frame_id);
...
if bb_isdef(tracker.bb)
    ...
    pattern = generate_pattern(dres_image.getFrameGray(frame_id), tracker.bb, tracker.patchsize);
