...
%
% MDP value function
function [tracker, qscore, f] = MDP_value_oop(tracker, frame_id, dres_image, dres_det, index_det)

% tracked, decide to tracked or occluded
if tracker.state == 2
    % extract features with LK tracking
    [tracker, f] = MDP_feature_tracked_oop(frame_id, dres_image, dres_det, tracker);
    ...
    if label > 0
        ...
        tracker = LK_update(frame_id, tracker, dres_image.getFrameGray(frame_id), dres_det, 0);
    else
        tracker.state = 3;
        dres_one.state = 3;
        tracker.dres = concatenate_dres(tracker.dres, dres_one);        
...
elseif tracker.state == 3
    ...
    if isempty(index_det) == 1
        ...
        [features, flag] = MDP_feature_occluded_oop(frame_id, dres_image, dres, tracker);
        ...
        tracker = LK_associate_oop(frame_id, dres_image, dres_one, tracker);
    end
    ...
    if label > 0
        ...
        tracker = LK_update(frame_id, tracker, dres_image.getFrameGray(frame_id), dres_det, 1);           
    else
        % no association
        tracker.state = 3;
        dres_one = sub(tracker.dres, numel(tracker.dres.fr));
